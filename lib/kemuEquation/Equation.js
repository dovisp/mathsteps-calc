...
class Equation {
  ...
  toString() {
    const rv =
      mathsteps.print(this.left.node) +
      " " +
      this.comparator +
      " " +
  ...
  getAsTeX() {
    const rv =
      mathsteps.printAsTeX(this.left.node) +
      " " +
      this.comparator +
      " " +
  ...
  getScheme() {
    const rv = this.left.toString() + this.comparator + this.right.toString();
    return rv;
  }
  ...
  _logStep(msg, showRepeatedSteps = true, ext = {}) {
    ...
    if (msg) {
      ...
      if (showRepeatedSteps || columnEq != this.logLastColumnEq) {
        ...
        this.logWidthForStepName = Math.max(
          this.logWidthForStepName,
          columnMsg.length,
        );
        this.logWidthForEquation = Math.max(
          this.logWidthForEquation,
          columnEq.length,
        );
  ...
  applyRules(poolOfRules, options = {}) {
    ...
    while (goOn) {
      ...
      const eqNode = new math.FunctionNode("EQ", [
        this.left.node,
        this.right.node,
      ]);
  ...
  applySolution(solutions) {
    ...
    this._logStep("solution", true, {
      arrayOfSolutions,
      arrayOfSolutionsSteps,
    });
  }
